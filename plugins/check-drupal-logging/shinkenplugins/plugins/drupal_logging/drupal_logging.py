...
class CheckDrupalLogging(ShinkenPlugin):
    NAME = 'drupal_logging'
    VERSION = '1.0'
    ...
    def _get_site_audit_result(self, path):
        try:
            ...
            return None, "Command 'drush --json aw' " \
    ...
    def _call_site_audit(self, path):
        out = subprocess.check_output(['drush', '--json', 'aw'], cwd=path)
        return json.loads(out)

    def parse_args(self, args):
        """ Use this function to handle complex conditions """
        args = super(CheckDrupalLogging, self).parse_args(args)
